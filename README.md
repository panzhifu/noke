...
| 已经进 3D | 点了会开面板 |
| --- | --- |
| 地毯(铺在书桌下)、书桌、桌上的显示器、桌边地板上的那台侧透机箱、床、唱机、冰箱、电竞椅,外加一间「墙角 + 地板」的舞台与一盏台灯 —— 见 `src/room/manifest.rs` | 书桌与桌上的显示器(开「作品」)、床(开「关于」) |
...
- 落点是**出图定了两版才定下来的**:先摆桌上右头那一格(x=0.58),实拍出来台灯灯罩正压在
  玻璃面中间、冰箱(1.34 m)又在镜头这一侧,右下整块被吃掉;换 x=0.78 只是少挡一点。
  最后落到书桌左前角的地板上 `(-0.85, 0, -0.8)` 转 -20° —— 玻璃面 + 正面三枚风扇同时入镜,
  离床架 0.27、离办公椅 0.10、贴着桌沿,四边都不相交
...
- 四态各出一张近景:白天两态玻璃里的 GPU 双风扇、水冷管、RGB 条都读得出来;
  夜两态整台是黑的、只剩三枚风扇的蓝光 —— 这模型本来就是黑机箱,没有自发光贴图可依赖
