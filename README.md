...
| 已经进 3D | 点了会开面板 |
| --- | --- |
| 地毯(铺在书桌下)、书桌、桌上的显示器、**塞在桌下右格的那台侧透机箱**、床、唱机、冰箱、电竞椅,外加一间「墙角 + 地板」的舞台与一盏台灯 —— 见 `src/room/manifest.rs` | 书桌与桌上的显示器(开「作品」)、床(开「关于」) |
...
**电竞椅点一下转一圈**:转的是**模型根节点**的 Y,360° 缓出停下(和冰箱门同一套缓出手感),连点会
排队、最多排三圈,`data-spin`(`spinning` / `idle`)与 `data-turns`(累计圈数)写在 `#room3d` 上
供核对。它进 `pickables` —— 代价是**椅背正后方那一小块桌面点不动了**
...
转椅要**原点在旋转轴(底盘立柱)上**。电竞椅的轴心不在包围盒中心(椅背往后伸,偏出 3cm),所以它的
glb 在压缩阶段就把原点挪到了底盘立柱上(骨架里 `wheelBase_10` 那根骨头,原型
`target/tmp/gaming_chair/pipe.py`,见「踩过的坑」)。
...
3. **在 `src/room/manifest.rs` 的 `MODELS` 里加一条**:路径、位置、旋转、缩放,以及三个可选的
   交互字段 —— `spot`(点了开哪格面板)、`door`(glb 里那个可开关的节点名)、`spin`(点一下转多少度)。
...
`door` 与 `spin` 是另外两种「点了会动」:`door` 给的是 **glb 里的节点名**(冰箱门那种,原点要在铰链上),
`spin` 给的是**每次点击转多少度**(转椅那种,原点要在旋转轴上)。两者都写了也不会互相打架 ——
...
模型(磁盘上的实际字节):电竞椅 2.39 MB / 机箱 2.36 MB / 床 2.07 MB / 唱机 1.30 MB /
冰箱 547 KB / 显示器 146 KB / 地毯 79 KB / 书桌 17 KB,合计 8.89 MB(gzip 都压不动,里面已经是 WebP)。
...
- 🔴 **转椅的旋转轴 = 底盘立柱,不是包围盒中心**。转的是模型根节点,所以 glb 的原点必须落在
  那根竖轴上,否则椅子是**绕着一根偏心的轴公转**,底座会画一个小圈。`tools/compress_glb.py`
  默认的「水平居中」取的是几何包围盒中心 —— 对办公椅恰好等于轴心(实测偏差 0.000 m,所以它
  加个 `spin` 就能转),但对椅背往后伸的转椅会偏出几厘米。**轴心能从骨架直接读**:导入后
  `arm.matrix_world @ pose.bones['wheelBase_10'].head` 就是底盘立柱的位置(比拿网格猜可靠),
  归一化时按它居中即可 —— 原型在 `target/tmp/gaming_chair/pipe.py`。
