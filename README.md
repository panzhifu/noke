...
**办公椅点一下转一圈**:转的是**模型根节点**的 Y,360° 缓出停下(和冰箱门同一套缓出手感),连点会
排队、最多排三圈,`data-spin`(`spinning` / `idle`)与 `data-turns`(累计圈数)写在 `#room3d` 上
供核对。它原先只是装饰、不进 `pickables`,现在会拾取 —— 代价是**椅背正后方那一小块桌面点不动了**
(书桌左右两侧照常开「作品」),这是明确选过的取舍:看得见的是椅子,点中椅子更合直觉。

这两件事对模型的要求不是包围盒能看出来的,都在清单字段的注释里写清了:门要**独立节点 + 原点在铰链上**,
转椅要**原点在旋转轴(底盘立柱)上**。办公椅的原点恰好就是「脚底贴地 + 水平居中」,而它的轴心也正是
包围盒中心(实测底盘中心与原点偏差 **0.000 m**),所以它加一个 `spin` 就能转;换个轴心偏心的模型就得
先在压缩阶段把原点挪到底盘上(见「踩过的坑」)。
