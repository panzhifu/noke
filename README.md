...
| 已经进 3D | 点了会开面板 |
| --- | --- |
| 地毯(铺在书桌下)、书桌、桌上的显示器 + 机械键盘 + 唱机(唱机上躺一张 12 寸黑胶)、**塞在桌下右格的那台侧透机箱**、床、冰箱、电竞椅,外加一间「墙角 + 地板」的舞台与一盏台灯 —— 见 `src/room/manifest.rs` | 书桌、显示器和键盘(都开「作品」)、床(开「关于」) |
...
3. **在 `src/room/manifest.rs` 的 `MODELS` 里加一条**:路径、位置、旋转、缩放,以及三个可选的
   交互字段 —— `spot`(点了开哪格面板)、`door`(glb 里那个可开关的节点名)、`spin`(glb 里那个要转的节点名)。
...
`door` 与 `spin` 是另外两种「点了会动」,**两个字段给的都是 glb 里的节点名**:`door` 是要开关的那块
(冰箱门,原点落在铰链上),`spin` 是要转的那个部件(电竞椅的 `chair_upper`,原点落在底盘立柱上)。
动多少、什么手感留在 `assets/room/config.js`(`DOOR_OPEN_DEG` / `DOOR_EASE` 与 `SPIN_DEG` / `SPIN_EASE`
一对一对应)—— 清单只管「点它动哪个节点」。两者都写了也不会互相打架 ——
`door` 优先。给了这三个字段里的任何一个,家具就进 `pickables`,会参与拾取。
