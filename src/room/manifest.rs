...
pub const MODELS: &[Model] = &[
    ...
    Model {
        ...
        position: (-0.85, 0.0, -0.8),
        // 玻璃面本来正对镜头(+Z),机箱的正面(三枚风扇)在 +X,斜 20° 让这两面同时入镜。
        //
        // 为什么在地板上而不是桌上:桌上只有左右两头有空,右头那格(台灯与桌沿之间)
        // 实拍出来被两样东西糊住 —— 台灯灯罩正好压在玻璃面中间,冰箱(1.34 高)又比
        // 「桌面 0.72 + 机箱 0.50」矮不了多少,它在镜头这一侧,于是机箱右下整块被吃掉。
        // 摆到书桌左前角的地板上反而干净:左边离床架(-1.38)还差 0.27,右边不挡办公椅
        // (椅脚到 -0.485),后边正好贴着桌沿(桌子进深到 -1.01)。
        // 转 20° 后 x 占 -1.108~-0.592、z 占 -1.003~-0.597,四边都不相交。
